...
if [[ -z "$solution" ]]; then
  solution="$RepoRoot/TerraFX.Interop.Windows.sln"
fi
...
if [[ ! -z "$architecture" ]]; then
  ...
  . "$DotNetInstallScript" --channel master --version 5.0.100-preview.4.20258.7 --install-dir "$DotNetInstallDirectory" --architecture "$architecture"
