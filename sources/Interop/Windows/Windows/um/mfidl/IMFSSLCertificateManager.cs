...
{
    static Guid* INativeGuid.NativeGuid => (Guid*)Unsafe.AsPointer(in IID_IMFSSLCertificateManager);
    ...
    {
        [VtblIndex(3)]
        HRESULT GetClientCertificate([NativeTypeName("LPCWSTR")] char* pszURL, byte** ppbData, [NativeTypeName("DWORD *")] uint* pcbData);

        [VtblIndex(4)]
        HRESULT BeginGetClientCertificate([NativeTypeName("LPCWSTR")] char* pszURL, IMFAsyncCallback* pCallback, IUnknown* pState);

        [VtblIndex(5)]
        HRESULT EndGetClientCertificate(IMFAsyncResult* pResult, byte** ppbData, [NativeTypeName("DWORD *")] uint* pcbData);

        [VtblIndex(6)]
        HRESULT GetCertificatePolicy([NativeTypeName("LPCWSTR")] char* pszURL, BOOL* pfOverrideAutomaticCheck, BOOL* pfClientCertificateAvailable);

        [VtblIndex(7)]
        HRESULT OnServerCertificate([NativeTypeName("LPCWSTR")] char* pszURL, byte* pbData, [NativeTypeName("DWORD")] uint cbData, BOOL* pfIsGood);
    }
