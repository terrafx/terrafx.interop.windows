...
// Ported from d3d12compatibility.h in microsoft/DirectX-Headers tag v1.618.5
...
{
    static Guid* INativeGuid.NativeGuid => (Guid*)Unsafe.AsPointer(in IID_ID3D12CompatibilityDevice);
    ...
    {
        [VtblIndex(3)]
        HRESULT CreateSharedResource([NativeTypeName("const D3D12_HEAP_PROPERTIES *")] D3D12_HEAP_PROPERTIES* pHeapProperties, D3D12_HEAP_FLAGS HeapFlags, [NativeTypeName("const D3D12_RESOURCE_DESC *")] D3D12_RESOURCE_DESC* pDesc, D3D12_RESOURCE_STATES InitialResourceState, [NativeTypeName("const D3D12_CLEAR_VALUE *")] D3D12_CLEAR_VALUE* pOptimizedClearValue, [NativeTypeName("const D3D11_RESOURCE_FLAGS *")] D3D11_RESOURCE_FLAGS* pFlags11, D3D12_COMPATIBILITY_SHARED_FLAGS CompatibilityFlags, ID3D12LifetimeTracker* pLifetimeTracker, ID3D12SwapChainAssistant* pOwningSwapchain, [NativeTypeName("const IID &")] Guid* riid, void** ppResource);

        [VtblIndex(4)]
        HRESULT CreateSharedHeap([NativeTypeName("const D3D12_HEAP_DESC *")] D3D12_HEAP_DESC* pHeapDesc, D3D12_COMPATIBILITY_SHARED_FLAGS CompatibilityFlags, [NativeTypeName("const IID &")] Guid* riid, void** ppHeap);

        [VtblIndex(5)]
        HRESULT ReflectSharedProperties(ID3D12Object* pHeapOrResource, D3D12_REFLECT_SHARED_PROPERTY ReflectType, void* pData, uint DataSize);
    }
