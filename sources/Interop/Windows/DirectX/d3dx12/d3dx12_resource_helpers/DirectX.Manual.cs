...
// Ported from d3dx12_resource_helpers.h in microsoft/DirectX-Headers tag v1.616.0
...
{
    public static void D3D12DecomposeSubresource<T, U, V>([NativeTypeName("UINT")] uint Subresource, [NativeTypeName("UINT")] uint MipLevels, [NativeTypeName("UINT")] uint ArraySize, [NativeTypeName("T &")] out T MipSlice, [NativeTypeName("T &")] out U ArraySlice, [NativeTypeName("T &")] out V PlaneSlice)
    ...
    public static ulong UpdateSubresources(ID3D12GraphicsCommandList* pCmdList, ID3D12Resource* pDestinationResource, ID3D12Resource* pIntermediate, uint FirstSubresource, uint NumSubresources, [NativeTypeName("UINT64")] ulong RequiredSize, [NativeTypeName("const D3D12_PLACED_SUBRESOURCE_FOOTPRINT *")] D3D12_PLACED_SUBRESOURCE_FOOTPRINT* pLayouts, [NativeTypeName("const UINT *")] uint* pNumRows, [NativeTypeName("const UINT64 *")] ulong* pRowSizesInBytes, [NativeTypeName("const D3D12_SUBRESOURCE_DATA *")] D3D12_SUBRESOURCE_DATA* pSrcData)
    {
        ...
        if ((IntermediateDesc.Dimension != D3D12_RESOURCE_DIMENSION_BUFFER) || (IntermediateDesc.Width < (RequiredSize + pLayouts[0].Offset)) || (RequiredSize > nuint.MaxValue) || ((DestinationDesc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER) && ((FirstSubresource != 0) || (NumSubresources != 1))))
        {
            return 0;
        }
    ...
    public static bool D3DX12GetCopyableFootprints([NativeTypeName("const D3D12_RESOURCE_DESC1 &")] in D3D12_RESOURCE_DESC1 ResourceDesc, uint FirstSubresource, uint NumSubresources, [NativeTypeName("UINT64")] ulong BaseOffset, D3D12_PLACED_SUBRESOURCE_FOOTPRINT* pLayouts, uint* pNumRows, [NativeTypeName("UINT64 *")] ulong* pRowSizeInBytes, [NativeTypeName("UINT64 *")] ulong* pTotalBytes)
    {
        ...
        ref readonly D3D12_RESOURCE_DESC1 resourceDesc = ref D3DX12ConditionallyExpandAPIDesc(ref LresourceDesc, in ResourceDesc);
        ...
        for (; uSubRes < NumSubresources; ++uSubRes)
        {
            ...
            D3D12_SUBRESOURCE_FOOTPRINT* Placement = (pLayouts != null) ? &pLayouts[uSubRes].Footprint : &LocalPlacement;
            Placement->Format = PlaneFormat;
            Placement->Width = PlaneWidth;
            Placement->Height = PlaneHeight;
            Placement->Depth = Depth;

            _ = D3D12_PROPERTY_LAYOUT_FORMAT_TABLE.CalculateMinimumRowMajorRowPitch(PlaneFormat, MinPlanePitchWidth, out uint MinPlaneRowPitch);

            Debug.Assert((D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT >= D3D12_TEXTURE_DATA_PITCH_ALIGNMENT) && ((D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT % D3D12_TEXTURE_DATA_PITCH_ALIGNMENT) == 0), "D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT must be >= and evenly divisible by D3D12_TEXTURE_DATA_PITCH_ALIGNMENT.");
            ...
            {
                _ = D3D12_PROPERTY_LAYOUT_FORMAT_TABLE.CalculateMinimumRowMajorRowPitch(PlaneFormat, PlaneWidth, out uint PlaneRowSize);
        ...
        if (pLayouts != null)
        {
            NativeMemory.Fill(pLayouts + uSubRes, (nuint)(sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT)) * (NumSubresources - uSubRes), byte.MaxValue);
        }
        if (pNumRows != null)
        {
            NativeMemory.Fill(pNumRows + uSubRes, (nuint)(sizeof(uint)) * (NumSubresources - uSubRes), byte.MaxValue);
        }
        if (pRowSizeInBytes != null)
        {
            NativeMemory.Fill(pRowSizeInBytes + uSubRes, (nuint)(sizeof(ulong)) * (NumSubresources - uSubRes), byte.MaxValue);
        }
        if (pTotalBytes != null)
        {
